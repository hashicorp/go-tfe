...
const testUnexpectedErrorFmt = "unexpected error: %v"

func TestClient_newClient(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", ContentTypeVndJSON)
...
func TestClient_headers(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("Accept") != ContentTypeVndJSON {
	...
	_, err = client.Plans.Logs(ctx, "plan-123456789")
	require.NoError(t, err)
	err = client.Runs.Apply(ctx, "run-123456789", RunApplyOptions{})
	require.NoError(t, err)
	_, err = client.Workspaces.Lock(ctx, "ws-123456789", WorkspaceLockOptions{})
	require.NoError(t, err)
	_, err = client.Workspaces.Read(ctx, "organization", "workspace")
	require.NoError(t, err)
...
func TestClient_requestBodySerialization(t *testing.T) {
	...
	t.Run("non-pointer request", func(t *testing.T) {
		body := InvalidBody{}
		_, err := createRequest(body)
		if err == nil || err != ErrInvalidRequestBody {
			t.Fatalf(testUnexpectedErrorFmt, err)
		}
	})
